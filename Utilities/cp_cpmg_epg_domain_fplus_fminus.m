%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  This script is a generalization of the EPG simulation code from        %
%  Matthias Weigel for multi-spin echo sequences where the refocusing     %
%  flip angle is an additional input parameter.                           %
%                                                                         %
%  (c) Matthias Weigel (2015)                                             %
%  https://github.com/matthias-weigel/EPG                                 %
%  http://epg.matthias-weigel.net                                         %
%  The code is based on the depiction and discussion of Extended Phase    %
%  Graphs in the following publication ("EPG-R"):                         %
...
%                                                                         %
%  Modified by our collaborator Tom Hilbert (TH, 2018)                    %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


function [F0_vector_out,Xi_F_out,Xi_Z_out,Xi_F_all_out,Xi_Z_all_out] = cp_cpmg_epg_domain_fplus_fminus (alpha_exc,N_in,alpha_in,ESP_in,T1_in,T2_in)
...
Omega_preRF  = complex(Omega_preRF,Omega_preRF); % TH for codegen
Omega_postRF = zeros(3,Nt2p1);                      % CP/CPMG shifts by +2 per interval
Omega_postRF  = complex(Omega_postRF,Omega_postRF); % TH for codegen
...
Xi_F_all_out  = complex(Xi_F_all_out,Xi_F_all_out); % TH for codegen
Xi_Z_all_out = zeros(Nt2p1,Nt2);                    % Xi_Z with Z states, upper half of Eq.[27b] in EPG-R
Xi_Z_all_out  = complex(Xi_Z_all_out,Xi_Z_all_out); % TH for codegen
...
%     Omega_postRF(2,1) = 1;                          % remember: the Omega state matrix contains both F+(0) and F-(0)=(F+(0))*   
	Omega_postRF(1,1) = -sin(-fa_exc);                % TH - Non perfect excitation pulse
    Omega_postRF(2,1) = -sin(-fa_exc);                % TH
	Omega_postRF(3,1) = cos(-fa_exc);                 % TH
else                                                % CP :
...
% ---------------------------------------------------------------------------------------------------------------------
T = zeros(3,3); %TH for codegen
T = complex(T,T);%TH for codegen
